...
# --- ЗАГОЛОВОК ---
st.markdown("<h2 style='text-align: center; padding-bottom: 20px;'>🌿 Агрохімічне обстеження ґрунтів</h2>", unsafe_allow_html=True)

# --- БЛОК ФІЛЬТРІВ (У стильному контейнері) ---
with st.container(border=True):
    st.markdown("#### 🎯 Налаштування відображення")
    ...
    with c2:
        ...
        sel_fields = st.multiselect("📍 Оберіть поля", field_list, default=[], placeholder="Показати всі поля...")
...
if not gdf_res.empty:
    ...
    m1.metric("Кількість зон/зразків", len(gdf_res))
    m2.metric("Загальна площа (га)", f"{total_area:,.2f}".replace(",", " "))
    m3.metric(f"Середнє: {UA_LABELS.get(value_col, value_col)}", f"{avg_val:.2f}")
    m4.metric("Розкид значень (Мін - Макс)", f"{min_val:.1f}  →  {max_val:.1f}")
    st.markdown("---")

# --- КАРТА ТА ТАБЛИЦЯ ---
t_map, t_data = st.tabs(["🗺️ Просторова карта", "📋 Аналітична таблиця"])
...
with t_map:
    if not gdf_res.empty:
        ...
        def style_f(feature):
            ...
            return {'fillColor': color, 'color': '#ffffff', 'weight': 1, 'fillOpacity': 0.8} # Тонші та світліші контури
        ...
        folium.GeoJson(
            ...
            tooltip=folium.GeoJsonTooltip(fields=t_fields, aliases=t_aliases, style="font-family: sans-serif; font-size: 13px;")
        ).add_to(m)
        
        st_folium(m, width="100%", height=600, returned_objects=[]) # returned_objects=[] пришвидшує роботу карти
...
with t_data:
    if not gdf_res.empty:
        ...
        # Красива таблиця з прогрес-баром для вибраного показника
        st.dataframe(
            df_table, 
            use_container_width=True, 
            height=550,
            hide_index=True, # Ховаємо непотрібний індекс
            column_config={
                UA_LABELS.get(value_col, value_col): st.column_config.ProgressColumn(
                    UA_LABELS.get(value_col, value_col),
                    help=f"Візуалізація значень {UA_LABELS.get(value_col, value_col)}",
                    format="%.2f",
    ...
    if detected_scale and detected_scale in GRADATIONS:
        ...
        for k in sorted(list(keys)):
            ...
            for g in GRADATIONS[detected_scale].get(k, GRADATIONS[detected_scale].get("default", [])):
                st.markdown(f"""<div class='legend-item'><div style='width:16px;height:16px;background-color:{g['color']};border-radius:4px;margin-right:10px;border:1px solid rgba(0,0,0,0.2)'></div><div class='legend-label'>{g['label']}</div></div>""", unsafe_allow_html=True)
